...
<h1 class="context-title"><a href={$node.url_alias|ezurl}>{$node_name|wash}</a>&nbsp;-&nbsp;{'OpenMagazine sort sub items [%children_count]'|i18n( 'extension/openmagazine',, hash( '%children_count', $children_count ) )}</h1>
...
<div class="object-right">
    {if and( $priority_sorting, $node_can_edit, $children_count )}
        <input id="ezwt-update-priority" class="defaultbutton" type="submit" name="UpdatePriorityButton" value="{'Update priorities'|i18n( 'design/standard/websitetoolbar/sort' )}" title="{'Apply changes to the priorities of the items in the list above.'|i18n( 'design/standard/websitetoolbar/sort' )}" />
