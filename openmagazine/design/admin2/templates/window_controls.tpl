...
{def $node_url_alias      = $node.url_alias
     ...
     $valid_tabs = array( $default_tab, 'details', 'translations', 'locations', 'relations', 'states', 'openmagazine' )
}
...
{if ezini( 'ExportClassSettings', 'MagazineSectionClassIdentifier', 'openmagazine.ini' )|contains( $node.class_identifier ) }    
    {* openmagazine *}
    <li id="node-tab-openmagazine" class="last{if $node_tab_index|eq('openamgazine')} selected{/if}">
        {if $tabs_disabled}
            <span class="disabled" title="{'Tab is disabled, enable with toggler to the left of these tabs.'|i18n( 'design/admin/node/view/full' )}">{'Ordering'|i18n( 'design/admin/node/view/full' )}</span>
        {else}
            <a href={concat( $node_url_alias, '/(tab)/openmagazine' )|ezurl} title="{'Show openmagazine overview.'|i18n( 'extension/openmagazine' )}">{'OpenMagazine'|i18n( 'extension/openmagazine' )}</a>
        {/if}
    </li>
{/if}
